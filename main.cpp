...
void sort_review_list(){
    int n = reviewList.size();
    for (int row = 0; row < n; ++row) {
        for (int col = 0; col < n - row - 1; ++col) {
            if (reviewList[col].index > reviewList[col+1].index) {

                ReviewQuestion temp = reviewList[col];
                reviewList[col] = reviewList[col + 1];
                reviewList[col + 1] = temp;
            }
        }
    }
}
   ...
   // for(int i = 0; i < 30; i++){
    //	if(PickedQuestions[i]){
	    	for(ReviewQuestion review: reviewList){
              //  if(review.index == i){
                    cout << "Question " << review.index << ": " << endl;
                    cout << review.question << endl;
                    cout << "\t\t\t\t\t"+review.name+"'s Answer: " << review.pAnswer << endl;
                    cout << "\t\t\t\t\tCorrect Answer: " << review.cAnswer;
                    cout << endl;
            //    }
	    	//}
	   // }
